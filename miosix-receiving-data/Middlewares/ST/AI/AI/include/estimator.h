...
#ifndef __AI_ESTIMATOR_H__
#define __AI_ESTIMATOR_H__
#pragma once

#include "ai_platform.h"

#define AI_ESTIMATOR_MODEL_NAME          "estimator"

#define AI_ESTIMATOR_IN_NUM       (1)
#define AI_ESTIMATOR_IN_1  \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 200, 1, NULL)
#define AI_ESTIMATOR_IN_1_SIZE \
  (1 * 1 * 200)

#define AI_ESTIMATOR_OUT_NUM      (1)
#define AI_ESTIMATOR_OUT_1  \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 2, 1, NULL)
#define AI_ESTIMATOR_OUT_1_SIZE \
  (1 * 1 * 2)
 ...
 * @ref ai_estimator_create. Both the activations memory buffer 
 ...
 * using @ref ai_estimator_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_ESTIMATOR_*
 * defines into file @ref estimator.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_estimator_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_ESTIMATOR_H__*/
